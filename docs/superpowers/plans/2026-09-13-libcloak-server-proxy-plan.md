...
2. Obligation 4: a client that PIPELINES its first data frame immediately
   behind its ClientHello, in the SAME write(), so those bytes are
   already in the kernel's receive buffer when conn_handoff does
   cloak_reactor_remove_fd followed by cloak_session_add_conn. Assert
   they arrive at the upstream. This is the "bytes survive the
   re-registration" property three branches have reasoned about on paper
   and none has tested. If it does NOT hold, that is a real bug in the
   hand-off and the single most valuable thing this plan will find --
...
- **The most valuable thing here** is Task 4's case 2. If the hand-off reasoning is wrong, every session whose client pipelines loses its first frame — silently, since the frame is simply never delivered and nothing anywhere reports an error.
