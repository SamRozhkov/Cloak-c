...
static void session_on_envelope(cloak_switchboard_t *sb, const uint8_t *frame_bytes, size_t frame_len, void *userdata) {
    ...
    if (found && state == CLOAK_STRMTAB_ACTIVE) {
        ...
        if (rc == 2) {
            /* A window update: credit, not data. Nothing to route and no
             * reader to notify -- but a producer that paused for want of
             * credit is waiting on exactly this, and on_writable is the
             * signal every relay already knows how to resume on. */
            if (sesh->on_writable != NULL) {
                sesh->on_writable(sesh, sesh->on_writable_userdata);
            }
            return;
