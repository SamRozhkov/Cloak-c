...
static void test_none_is_passthrough(void) {
    ...
    int rc = cloak_aead_seal(CLOAK_AEAD_NONE, key, nonce, NULL, 0, plaintext, sizeof(plaintext), out, &out_len);
    ASSERT_EQ_INT(rc, 0);
...
static void test_aes256gcm_round_trip(void) {
    ...
    int rc = cloak_aead_seal(CLOAK_AEAD_AES_256_GCM, key, nonce, NULL, 0, plaintext, sizeof(plaintext),
    ...
    rc = cloak_aead_open(CLOAK_AEAD_AES_256_GCM, key, nonce, NULL, 0, ciphertext, ciphertext_len,
...
static void test_aes256gcm_tamper_detection(void) {
    ...
    uint8_t ciphertext[sizeof(plaintext) + CLOAK_AEAD_TAG_LEN];
    size_t ciphertext_len = 0;
    cloak_aead_seal(CLOAK_AEAD_AES_256_GCM, key, nonce, NULL, 0, plaintext, sizeof(plaintext),
                     ciphertext, &ciphertext_len);
    ...
    uint8_t decrypted[sizeof(plaintext)];
    size_t decrypted_len = 0;
    int rc = cloak_aead_open(CLOAK_AEAD_AES_256_GCM, key, nonce, NULL, 0, ciphertext, ciphertext_len,
                              decrypted, &decrypted_len);
    ASSERT_EQ_INT(rc, -1);
}
...
static void test_aes256gcm_wrong_key_fails(void) {
    ...
    uint8_t decrypted[sizeof(plaintext)];
    size_t decrypted_len = 0;
    int rc = cloak_aead_open(CLOAK_AEAD_AES_256_GCM, wrong_key, nonce, NULL, 0, ciphertext, ciphertext_len,
                              decrypted, &decrypted_len);
    ASSERT_EQ_INT(rc, -1);
}
...
static void round_trip_for_method(cloak_aead_method_t method) {
    ...
    int rc = cloak_aead_seal(method, key, nonce, NULL, 0, plaintext, sizeof(plaintext), ciphertext, &ciphertext_len);
    ASSERT_EQ_INT(rc, 0);
    ASSERT_EQ_INT(ciphertext_len, sizeof(plaintext) + CLOAK_AEAD_TAG_LEN);

    uint8_t decrypted[sizeof(plaintext)];
    size_t decrypted_len = 0;
    rc = cloak_aead_open(method, key, nonce, NULL, 0, ciphertext, ciphertext_len, decrypted, &decrypted_len);
    ASSERT_EQ_INT(rc, 0);
    ASSERT_EQ_INT(decrypted_len, sizeof(plaintext));
    ASSERT_MEM_EQ(decrypted, plaintext, sizeof(plaintext));
}
...
static void test_unknown_method_fails(void) {
    ...
    int rc = cloak_aead_seal((cloak_aead_method_t)99, key, nonce, NULL, 0, plaintext, sizeof(plaintext), out, &out_len);
    ASSERT_EQ_INT(rc, -1);
}
...
static void test_aes128gcm_ignores_key_bytes_16_31(void) {
    ...
    int rc = cloak_aead_seal(CLOAK_AEAD_AES_128_GCM, key_a, nonce, NULL, 0, plaintext, sizeof(plaintext),
                              ciphertext_a, &ciphertext_a_len);
    ASSERT_EQ_INT(rc, 0);
    rc = cloak_aead_seal(CLOAK_AEAD_AES_128_GCM, key_b, nonce, NULL, 0, plaintext, sizeof(plaintext),
...
static void test_aead_with_aad_round_trip(void) {
    uint8_t key[CLOAK_AEAD_KEY_LEN];
    uint8_t nonce[CLOAK_AEAD_NONCE_LEN];
    cloak_random_bytes(key, sizeof(key));
    cloak_random_bytes(nonce, sizeof(nonce));
    ...
    const uint8_t plaintext[] = "aad-protected message";
    uint8_t ciphertext[sizeof(plaintext) + CLOAK_AEAD_TAG_LEN];
    size_t ciphertext_len = 0;

    int rc = cloak_aead_seal(CLOAK_AEAD_AES_256_GCM, key, nonce, aad, sizeof(aad),
                              plaintext, sizeof(plaintext), ciphertext, &ciphertext_len);
    ASSERT_EQ_INT(rc, 0);

    uint8_t decrypted[sizeof(plaintext)];
    size_t decrypted_len = 0;
    rc = cloak_aead_open(CLOAK_AEAD_AES_256_GCM, key, nonce, aad, sizeof(aad),
...
static void test_aead_tampered_aad_rejected(void) {
    uint8_t key[CLOAK_AEAD_KEY_LEN];
    uint8_t nonce[CLOAK_AEAD_NONCE_LEN];
    cloak_random_bytes(key, sizeof(key));
    cloak_random_bytes(nonce, sizeof(nonce));
    ...
    const uint8_t aad[] = {0x01, 0x02};
    const uint8_t wrong_aad[] = {0x01, 0x03};
    const uint8_t plaintext[] = "secret";
    uint8_t ciphertext[sizeof(plaintext) + CLOAK_AEAD_TAG_LEN];
    size_t ciphertext_len = 0;

    cloak_aead_seal(CLOAK_AEAD_AES_256_GCM, key, nonce, aad, sizeof(aad),
                     plaintext, sizeof(plaintext), ciphertext, &ciphertext_len);

    uint8_t decrypted[sizeof(plaintext)];
    size_t decrypted_len = 0;
    int rc = cloak_aead_open(CLOAK_AEAD_AES_256_GCM, key, nonce, wrong_aad, sizeof(wrong_aad),
                              ciphertext, ciphertext_len, decrypted, &decrypted_len);
    ASSERT_EQ_INT(rc, -1);
}

static void test_aead_none_ignores_aad(void) {
    uint8_t key[CLOAK_AEAD_KEY_LEN] = {0};
    uint8_t nonce[CLOAK_AEAD_NONCE_LEN] = {0};
    const uint8_t aad[] = {0xAA, 0xBB};
    const uint8_t plaintext[] = "plain passthrough";
    uint8_t out[sizeof(plaintext)];
    size_t out_len = 0;

    int rc = cloak_aead_seal(CLOAK_AEAD_NONE, key, nonce, aad, sizeof(aad),
                              plaintext, sizeof(plaintext), out, &out_len);
    ASSERT_EQ_INT(rc, 0);
    ASSERT_EQ_INT(out_len, sizeof(plaintext));
    ASSERT_MEM_EQ(out, plaintext, sizeof(plaintext));
}
...
TEST_MAIN_BEGIN()
    ...
    test_aead_with_aad_round_trip();
    test_aead_tampered_aad_rejected();
    test_aead_none_ignores_aad();
TEST_MAIN_END()
